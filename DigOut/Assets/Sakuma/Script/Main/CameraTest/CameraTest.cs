...
public class CameraTest : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (!stepMove) {
            ...
            if (!Physics2D.CircleCast(transform.position, 0.4f, new Vector2(move.x, 0), dis, mask)) {
            ...
            else
            {
                ...
                {
                    cont -= 1f;
                    if (!Physics2D.CircleCast(transform.position, 0.4f, new Vector2(move.x, 0), dis * (cont / 10f), mask))
                    {
                        flg = false;
                    }
                    if (cont <= 0)
            ...
            if (!Physics2D.CircleCast(transform.position, 0.4f, new Vector2(0, move.y), dis, mask)) {
            ...
            else
            {
                ...
                {
                    cont -= 1f;
                    if (!Physics2D.CircleCast(transform.position, 0.4f, new Vector2(0, move.y), dis * (cont / 10f), mask))
                    {
                        flg = false;
                    }
                    if (cont <= 0)
            ...
            if (!Physics2D.CircleCast(popObj.transform.position, 0.4f, new Vector2(move.x, 0), dis, mask)) {
                popRigidbody2D.transform.Translate(new Vector2(move.x, 0) * dis);
            }

            if (!Physics2D.CircleCast(popObj.transform.position, 0.4f, new Vector2(0, move.y), dis, mask)) {
